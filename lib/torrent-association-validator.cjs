...
function classifyTorrentAssociation(row, metadata) {
  ...
  if (!hasAliasMatch(candidate, aliases)) {
    return {
      status: years.length > 0 ? 'invalid' : 'review',
      reason: years.length > 0 ? 'title does not match any metadata alias' : 'title mismatch without year evidence',
      confidence: years.length > 0 ? 'high' : 'medium'
    };
  }
