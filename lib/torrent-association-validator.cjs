...
// Do not include `.com`: release names frequently end with tracker/site domains
// (for example SceneTime.com), which are not DOS executable filenames.
const NON_VIDEO_EXTENSIONS = /\.(?:exe|scr|bat|cmd|msi|apk|dmg|pkg|zip|rar|7z|epub|pdf|flac|mp3|wav)(?:\s|$)/i;
...
function classifyTorrentAssociation(row, metadata) {
  ...
  if (expectedYear && years.length > 0 && !hasExpectedYear) {
    ...
    return {
      status: isSeries ? 'review' : 'invalid',
      reason: isSeries
        ? `series year differs (${[...new Set(years)].join(', ')}; first release ${expectedYear})`
        : `conflicting year (${[...new Set(years)].join(', ')}; expected ${expectedYear})`,
      confidence: isSeries ? 'medium' : 'high'
    };
  }
